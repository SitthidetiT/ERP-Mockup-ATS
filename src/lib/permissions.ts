...
export function moduleForPath(pathname: string): ModuleKey | null {
  ...
  if (
    ...
    pathname.startsWith("/public/quotation")
  )
    return "quotations";
...
export function canAccess(role: Role, pathname: string) {
  ...
  return moduleKey === null || rolePermissions[role].includes(moduleKey);
}
...
const writableModules: Record<Role, ModuleKey[]> = {
  ...
  Packing: ["packing"],
  Shipping: ["delivery"],
  Accounting: ["accounting", "costing", "reports"],
  HR: ["hr"],
};
...
export function canViewFinancials(role: Role) {
  return (
    role === "Executive" || role === "System Admin" || role === "Accounting"
  );
}
...
export function canMoveJobTo(role: Role, target: Status) {
  ...
  const targets: Partial<Record<Role, Status[]>> = {
    ...
    "Store / Warehouse": ["PRODUCTION"],
    "Production Supervisor": ["QC"],
    "Production Operator": ["QC"],
    QC: ["REWORK", "PACKING", "QC"],
    Packing: ["DELIVERED"],
    Shipping: ["COMPLETED"],
  };
