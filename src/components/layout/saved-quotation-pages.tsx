...
export function SavedQuotationDetailPage({ id }: { id: string }) {
  ...
  return <>
    <header className="page-header saved-quote-header"><div><h1>{quotation.no} · REV {quotation.rev}</h1><p>{quotation.customer} · {quotation.project}</p></div><div className="head-actions"><Link className="secondary" href={`/quotations/${quotation.id}/preview`}>เปิด A4 เต็มหน้า</Link><button className="primary" onClick={() => setQuoteStatus(quotation.id, "SENT")}>ส่งใบเสนอราคาทางอีเมล</button></div></header>
    <div className="saved-quotation-layout">
      ...
      <aside className="panel totals saved-quotation-sidebar"><h3>สรุปใบเสนอราคา</h3><div><span>สถานะ</span><span className="badge info">{quotation.status}</span></div><div><span>รวมก่อน VAT</span><b>{thb(total)}</b></div><div><span>VAT 7%</span><b>{thb(total * .07)}</b></div><div className="grand"><span>รวมสุทธิ</span><b>{thb(total * 1.07)}</b></div><button className="secondary fullbutton" onClick={() => router.push(`/public/quotation/${quotation.id}`)}>เปิดหน้าลูกค้า</button>{quotation.status !== "SENT" && <button className="primary fullbutton" onClick={() => setQuoteStatus(quotation.id, "SENT")}>ยืนยันส่งอีเมล</button>}{quotation.status !== "ACCEPTED" && <button className="secondary fullbutton" onClick={() => { setQuoteStatus(quotation.id, "ACCEPTED"); router.push("/jobs"); }}>ลูกค้ายอมรับและสร้างงาน</button>}</aside>
    </div>
  </>;
}
